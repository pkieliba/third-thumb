...
#% (left hemisphere registered to right hemisphere), resamples surface-based anatomical ROIs to the individual's
...
surfDir=/vols/Data/soma/6Finger/surf
export SUBJECTS_DIR="${surfDir}"
subj="$1"

# check whether subj dir exists, if not create it
if [ ! -d "${SUBJECTS_DIR}"/"${subj}" ]; then
    sessions=`ls -d /vols/Data/soma/6Finger/"${subj}"/[p]*`  #assumes that names of all scanning sessions start with "p", e.g. pre, post, postpost
    sesArray=(${sessions//\\n/})
    # create subj dir
...
if [ ! -f "${SUBJECTS_DIR}"/"${subj}"/surf/lh.pial ]; then
    ...
    # create giftis
    giftiID=`fsl_sub -q short.q -j $reconID /vols/Data/soma/6Finger/scripts/fsCreateGifti.sh "${subj}" "${surfDir}"/"${subj}"/gifti`
    # resample atlas into LHemi
    resampID=`fsl_sub -q short.q -j $giftiID /vols/Data/soma/6Finger/scripts/fsSurf2Surf.sh "${subj}" /vols/Data/soma/Atlases/Joern /vols/Data/soma/Atlases/Joern/ROI.label.gii L "${surfDir}"/"${subj}"/gifti`
    # project surf L.ROI to volume
    projID=`fsl_sub -q short.q -j $resampID /vols/Data/soma/6Finger/scripts/fsSurfProjBackL.sh "${subj}" L "${surfDir}"/"${subj}"/gifti/ROI/"${subj}".ROI.L.label.gii`
    ...
    xhemID=$(/vols/Data/soma/6Finger/scripts/submit-matlab.sh /vols/Data/soma/6Finger/scripts/logs/Xhem"${subj}" long.q Xhemi $reconID)
    # resample atlas into RHemi
    resampXhemID=`fsl_sub -q short.q -j "${xhemID}","${resampID}" /vols/Data/soma/6Finger/scripts/fsXhemi.sh "${subj}" "${surfDir}"/"${subj}"/gifti/ROI/"${subj}".ROI.L.label.gii lr`
    ...
    if [ ! -f "${SUBJECTS_DIR}"/"${subj}"/gifti/"${subj}".L.midthickness.surf.gii ]; then
      # create giftis
      giftiID=`fsl_sub -q short.q /vols/Data/soma/6Finger/scripts/fsCreateGifti.sh "${subj}" "${surfDir}"/"${subj}"/gifti`
      # resample atlas into LHemi
      resampID=`fsl_sub -q short.q -j $giftiID /vols/Data/soma/6Finger/scripts/fsSurf2Surf.sh "${subj}" /vols/Data/soma/Atlases/Joern /vols/Data/soma/Atlases/Joern/ROI.label.gii L "${surfDir}"/"${subj}"/gifti`
      ...
      projID=`fsl_sub -q short.q -j $resampID /vols/Data/soma/6Finger/scripts/fsSurfProjBackL.sh "${subj}" L "${surfDir}"/"${subj}"/gifti/ROI/"${subj}".ROI.L.label.gii`
        ...
        if [ ! -f "${SUBJECTS_DIR}"/"${subj}"/gifti/ROI/"${subj}".ROI.L.label.gii ]; then
            resampID=`fsl_sub -q short.q /vols/Data/soma/6Finger/scripts/fsSurf2Surf.sh "${subj}" /vols/Data/soma/Atlases/Joern /vols/Data/soma/Atlases/Joern/ROI.label.gii L "${surfDir}"/"${subj}"/gifti`
            # project surf ROI to volume
            projID=`fsl_sub -q short.q -j $resampID /vols/Data/soma/6Finger/scripts/fsSurfProjBackL.sh "${subj}" L "${surfDir}"/"${subj}"/gifti/ROI/"${subj}".ROI.L.label.gii`
        else
            # check if ROI projected to Volume
            if [ ! -d "${SUBJECTS_DIR}"/"${subj}"/gifti/Volume ]; then
                 fsl_sub -q short.q /vols/Data/soma/6Finger/scripts/fsSurfProjBackL.sh "${subj}" L "${surfDir}"/"${subj}"/gifti/ROI/"${subj}".ROI.L.label.gii
            fi
        fi
    fi
